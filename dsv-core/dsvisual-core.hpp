...
private:
    PlatformManager() {
        //std::lock_guard<std::mutex> _al(__mWindowOwnship);

        __mWindowDestroy = false;
...
private: // bigfive
    ...
    ~Window() {
        PlatformManager::getInstance().setWindowDestroy();
    }
...
public:
    static Window & getInstance() {
        //PlatformManager::getInstance(); // need before contruct w
        static Window w;
        //PlatformManager::getInstance(); // need before contruct w
        return w;
    }
...
void Window::render() {
    while (true) {
        ...
        if (PlatformManager::getInstance().getWindow() == nullptr) return; // TODO
